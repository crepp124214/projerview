...
| `projerview jd-match` | 分析 JD 与项目的匹配度 |
| `projerview jd-compare` | 多个 JD 对比，找出最优选择 |
| `projerview question-bank` | 重新生成面试题库 |
| `projerview mock-interview` | 直接进入模拟面试 |
| `projerview record` | 记录真实面试题目，分析命中率 |
| `面试准备` / `模拟面试` / `分析 JD` / `记录面试` | 中文触发词 |
...
粘贴目标岗位 JD，自动提取技术关键词并与项目分析交叉匹配。生成匹配度报告，标注高风险面试问题。支持多 JD 对比矩阵，一眼看出哪家公司准备最充分。输出 `.projerview/jd-match.md`。

### 阶段2：题库生成

基于面试手册（和 JD 匹配结果），生成面试题库 `.projerview/question-bank.md`。

核心题库 25-35 题，扩展题库不限。包含 8 个类别：项目介绍、架构设计、技术深挖、选型对比、场景题、难点挑战、反思改进、**行为面试题（STAR）**。难度分层：⭐基础 ~15% / ⭐⭐中等 ~60% / ⭐⭐⭐进阶 ~25%。每道题附带口语化参考答案、常见追问、关联八股文知识点，并按面试阶段分类速查。

### 阶段3：模拟面试 + 真实面试记录

Skill 扮演面试官，逐题提问、评分、追问。四种模式：全量/专题/随机/压力面试。评分维度：准确性(40%) + 深度(35%) + 表达(25%)。支持暂停保存进度、历史成绩对比、断点续面。

面试结束后可记录真实面试被问题目，与题库比对命中率，自动补充未覆盖的题目，让题库越用越准。
...
### v1.2.0 (2026-05-27)
- **JD 匹配分析**：粘贴目标岗位 JD，自动与项目分析交叉匹配，生成匹配度报告和高风险面试问题预测
- **多 JD 对比**：同时分析多个 JD，生成对比矩阵，找出匹配度最高的岗位
- **行为面试题**：新增 STAR 行为题类别，从 git 历史和 handbook 中自动提取真实素材
- **面试记录与复盘**：记录真实面试被问题目，与题库比对命中率，自动补充未覆盖题目
