---
name: projerview
description: 分析项目代码生成面试手册、出题、模拟面试、JD 匹配分析。面向计算机应届生，帮助深度理解简历项目并自信应对面试。当用户提到 projerview、面试准备、模拟面试、项目面试、面试题库、JD 匹配、分析 JD、记录面试时使用此技能。
---
...
步骤4: 如果用户要进入阶段3（模拟面试，项目类模式），检查简历来源
  - 优先级1：.projerview/resume.md 存在 → 使用
  - 优先级2：项目目录下存在简历文件（resume.md / resume.txt / resume.pdf / 简历.*）→ 读取并提取项目经历
  - 优先级3：都不存在 → 询问用户提供简历路径，或从 handbook 反推
```
...
| 用户指定 jd-match / jd-compare | 阶段1.5（JD 匹配分析） | 最高 |
...
| 用户指定 record / 记录面试 | 面试记录与复盘（读取 interview-report 后进入） | 最高 |
| .projerview/ 不存在 | 阶段0（预检）→ 阶段1 | 自动 |
| handbook.md 不存在 | 阶段0（预检）→ 阶段1 | 自动 |
| handbook.md 存在，question-bank.md 不存在 | 询问用户：审阅手册 / 分析 JD / 直接生成题库？→ 阶段1.5 或 阶段2（同时生成 question-bank.md + fundamentals.md） | 自动 |
| handbook.md + question-bank.md 存在，interview-report.md 不存在 | 阶段3 | 自动 |
| interview-report.md 存在 | 询问：重新面试 / 记录真实面试 / 反馈闭环？ | 自动 |
| progress.json 存在且 state≠completed | 提示用户可断点续面 | 自动 |
| progress.json 中 jd_match_state 存在且未完成 | 提示用户可继续 JD 分析 | 自动 |
...
| 面试准备 | 无（走自动检测） |
| 模拟面试 | mock-interview |
| 分析 JD / 匹配岗位 / JD 匹配 | jd-match |
| JD 对比 / 多个 JD | jd-compare |
| 记录面试 / 面试复盘 | record |
...
- 阶段1.5：读取 [references/jd-match.md](references/jd-match.md) 并执行（JD 匹配分析，可选）
...
| JD 内容过短（<50 字）或无法提取关键词 | 提示"JD 信息不足，请提供完整的岗位描述" |
| 用户提供的 URL 无法访问 | 提示用户直接粘贴 JD 文本内容 |
